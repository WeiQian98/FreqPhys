...
def run_training(lr, dim, depth, heads, mlpdim):
    ...
    input_path = f'/home/anonymous/data/{args.dataset}_MST/'
    ...
    if args.dataset == "VIPL":
        model_configs = f"Fold_{args.fold}_" + model_configs
    ...
    with open(log_file, 'w') as f:
        f.writelines([f"model=FreqPhys, Dataset={args.dataset}, Train_bs={args.Train_batchsize}, Test_bs={args.Test_batchsize}, learning_rate={lr}, k_times={config.K},  total_steps={config.Total_steps}\n",
                      "==============================\n"])
        f.flush()

    train, test, transform_train, transform_test = train_test_split(args.dataset, args.fold, input_path)

    dataset_train = DataLoader(train, args.dataset, transforms=transform_train)
    ...
    for epoch in range(args.EPOCHS):
        ...
        # testing
        if epoch%2==0:
            model_eval_temp = diff_model.main(
                image_height=63,
                image_width=300,
                dim=dim,
                depth=depth,
            ...
            val_target_hr_per_batch, val_predicted_hr_per_batch, val_target_bvp_per_batch, val_predicted_bvp_per_batch, val_loss_per_batch, path_per_batch = engine.test_fn(
                model_eval_temp, test_loader, lossfunc_pearson, lossfunc_mse, args.dataset)
    ...
    with open(log_file, "a") as f:
        f.writelines([f"MIN MAE OF {args.EPOCHS} epochs: {min_mae}  ",
                        f"MIN RMSE OF {args.EPOCHS} epochs: {min_rmse}\n"])
        f.flush()
...
if __name__ == "__main__":
    ...
    parser.add_argument('--Train_batchsize', type=int, default=12, help='Train_batchsize')
    parser.add_argument('--Test_batchsize', type=int, default=12, help='Test_batchsize')
            ...
            for depth in Depth:
                for heads in Heads:
                    for mlpdim in Mlpdim:
                        run_training(lr, dim, depth, heads, mlpdim)
